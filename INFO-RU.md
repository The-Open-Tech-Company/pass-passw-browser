...
- **PIN-код защита**: Доступ к паролям защищён PIN-кодом (6-12 символов, должен содержать хотя бы одну цифру и одну букву)
...
### 12. Обнаружение дублирующихся паролей
...
### 13. Биометрическая аутентификация (WebAuthn)

- **Поддержка платформ**: Работает с Windows Hello, Touch ID, Face ID и другими WebAuthn-совместимыми аутентификаторами
- **Процесс настройки**: 
  - Включите биометрическую аутентификацию в настройках расширения
  - Зарегистрируйте биометрические учётные данные с помощью аутентификатора устройства
  - PIN-код шифруется и хранится безопасно для биометрической разблокировки
- **Методы разблокировки**:
  - Разблокировка попапа расширения с помощью биометрической аутентификации
  - Автоматический запрос биометрии при доступности
  - Возврат к PIN-коду, если биометрия недоступна или не удалась
- **Безопасность**:
  - PIN-код зашифрован биометрическим ключом
  - Биометрические учётные данные хранятся локально
  - Биометрические данные не передаются на внешние серверы
- **Совместимость**: Требуется поддержка WebAuthn в браузере (Chrome 67+, Edge 18+)

### 14. Управление кодами TOTP/2FA

- **Поддержка TOTP**: Полная поддержка Time-based One-Time Password (RFC 6238)
- **Добавление кодов 2FA**:
  - Добавление TOTP секретов из приложений-аутентификаторов
  - Поддержка форматов Base32 и Hex для секретов
  - Связывание с названием сервиса и логином
- **Генерация кодов**:
  - Автоматическая генерация 6-значных кодов
  - Временной шаг 30 секунд (стандартный TOTP)
  - Обновление кодов в реальном времени
  - Визуальный таймер обратного отсчёта
- **Функции управления**:
  - Просмотр всех сохранённых кодов 2FA в попапе расширения
  - Редактирование названия сервиса, логина и секрета
  - Удаление кодов 2FA
  - Копирование в буфер обмена одним кликом
- **Безопасность**:
  - Все TOTP секреты зашифрованы PIN-кодом
  - Секреты никогда не хранятся в открытом виде
  - Безопасная производная ключа для шифрования
- **Интеграция**: Отдельная вкладка TOTP в попапе расширения для удобного доступа
...
- **Режим для больных эпилепсией**:
  ...
  - Упрощает элементы интерфейса
  - Предотвращает триггеры приступов
  - Автоматически переключается на светлую тему
- **Управление размером шрифта**:
  - Четыре размера: Маленький, Средний, Большой, Очень большой
  - Применяется ко всем элементам интерфейса
  - Улучшает читаемость для пользователей с нарушениями зрения
- **Режим высокого контраста**:
  - Улучшенные коэффициенты цветового контраста
  - Лучшая видимость для пользователей с ослабленным зрением
  - Соответствие стандарту WCAG AA
- **Уменьшение движения**:
  - Учитывает системную настройку `prefers-reduced-motion`
  - Отключает анимации и переходы
  - Снижает проблемы с чувствительностью к движению
- **Индикаторы фокуса**:
  - Всегда видимые контуры фокуса
  - Улучшенная поддержка клавиатурной навигации
  - Чёткая визуальная обратная связь для сфокусированных элементов
- **Поддержка ARIA**:
  - Правильные ARIA метки и роли
  - Live regions для объявлений динамического контента
  - Совместимость с программами чтения с экрана
- **Клавиатурная навигация**:
  - Полная поддержка клавиатуры для всех функций
  - Логический порядок табуляции
  - Горячие клавиши где уместно
- **Поддержка программ чтения с экрана**:
  - Семантическая HTML структура
  - Описательные метки и альтернативный текст
  - Объявления статуса через ARIA live regions

Все настройки доступности можно настроить в настройках расширения в разделе "Адаптация" и применяются немедленно без перезагрузки страницы.
...
- **Структура**: 
  ...
  - `biometricCredentialId`: ID учётных данных WebAuthn для биометрической аутентификации
  - `biometricCredentialData`: зашифрованные данные биометрических учётных данных
  - `biometricEnabled`: флаг включения биометрической аутентификации
  - `biometricEncryptedPin`: PIN-код, зашифрованный биометрическим ключом
  - `biometricPinKey`: ключ шифрования для биометрического PIN-кода
  - `totpList`: массив зашифрованных TOTP секретов
...
- **WebAuthn API**: Для биометрической аутентификации
...
Мы приветствуем вклад в проект! Пожалуйста, см. основной [README-RU.md](../README-RU.md) для руководящих принципов по внесению вклада.

### Безопасность

Если вы обнаружили уязвимость безопасности, пожалуйста, не создавайте публичный issue. Вместо этого свяжитесь с сопровождающими напрямую.
